...
### 답변 구조
1. **핵심 답변** (2-3문장): 질문에 대한 직접적이고 구체적인 답변
2. **근거 데이터** (bullet 2-4개): 
   - 관련 수치를 구체적으로 인용
...
### 답변 지시사항
1. **질문 이해**: 위 질문의 핵심 키워드와 의도를 파악하세요.
   - 질문 유형: 비교/트렌드/원인분석/개선제안/수치조회 중 무엇인가?
   - 언급된 키워드: 채널, 페이지, 메트릭, 기간 등
   - 묻는 것: 수치? 추세? 원인? 개선방법?

2. **관련 데이터 추출**: 질문과 직접 관련된 데이터만 선별하세요.
   - 질문에 언급된 메트릭/채널/페이지와 관련된 데이터
   - 비교가 필요하면 비교 대상 데이터
   - 트렌드가 필요하면 dailyTrend 데이터
   - 전체 데이터를 나열하지 말고 **질문에 답하는 데 필요한 데이터만** 사용

3. **구체적 답변**: 추출한 데이터를 기반으로 질문에 직접 답변하세요.
   - 구체적인 수치 인용 (예: "Organic Search는 1,234세션")
   - 데이터 소스 명시 (GA4/CSV/통합)
   - 비교나 추세 설명 (증가/감소, 비율 등)

4. **유의미한 후속 질문**: 답변 내용과 자연스럽게 연결되는 질문 1-2개를 제시하세요.
   - 더 깊은 분석을 위한 질문
   - 다른 관점이나 비교 질문
   - 실행 가능한 다음 단계 질문

**중요**: 리포트 전체를 반복하지 말고, 질문에 대한 핵심 답변만 간결하게 제공하세요."""
